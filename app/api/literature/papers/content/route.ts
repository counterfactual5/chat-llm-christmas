...
/**
 * GET /api/literature/papers/content?identifier=…
 * Proxy OA PDF bytes (no Files write) for ephemeral Preview.
 */
export async function GET(req: NextRequest) {
  ...
  try {
    const upstreamUrl = new URL(chatBackendLiteratureURL('papers/content'));
    ...
    if (!upstream.ok || !contentType.includes('pdf')) {
      const data = await upstream.json().catch(async () => {
        ...
        return { error: text.slice(0, 400) || `Paper content failed (HTTP ${upstream.status})` };
      });
