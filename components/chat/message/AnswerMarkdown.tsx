...
const REMARK_STREAMING = [remarkGfm];
const REMARK_SETTLED = [remarkMath, remarkGfm];
const REHYPE_SETTLED = [[rehypeKatex, KATEX_OPTIONS]] as const;
const REHYPE_NONE: never[] = [];

type AnswerMarkdownCtxValue = {
  streaming: boolean;
...
const AnswerMarkdownCtx = createContext<AnswerMarkdownCtxValue>({
  streaming: false,
});
...
/** Module-stable renderers — new `components={{}}` each render remounts imgs. */
const markdownComponents = {
  ...
  code({ className, children, ...props }: any) {
    const { streaming, onSendCommand } = useContext(AnswerMarkdownCtx);
...
export function AnswerMarkdown({
  ...
  const remarkPlugins = streaming ? REMARK_STREAMING : REMARK_SETTLED;
  const rehypePlugins = streaming ? REHYPE_NONE : REHYPE_SETTLED;

  const ctx: AnswerMarkdownCtxValue = {
    streaming,
    onSendCommand,
    onPreviewLink,
    previewBaseUrl,
  ...
  return (
    <AnswerMarkdownCtx.Provider value={ctx}>
      <div
        ref={rootRef}
        className={cn(
