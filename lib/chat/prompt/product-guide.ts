...
export function productUsageGuidePrompt(): string {
  return [
    ...
    'Commands (composer “/”, sidebar Commands): /image, /research, /papers, /books, /skill (singular; Skill Creator), /review (Request review / 请求审查), Continue reply / 继续回复. Full flags and args are only in the detailed guide when the user asks how to use the product.',
    ...
    'Tools (only if present in THIS-turn API tools): create_file + create_spreadsheet + web_search/web_read are built-in; paper_search / book_search / generate_image are opt-in toggles.',
...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- /research [standard|rigorous] [web|literature|mixed] <query>: Deep Research job (not an ordinary chat tool). Modes control depth (standard default; rigorous = more sources / stricter checks). Prefer /papers or /books for dedicated literature search/download.',
    '- /papers / /books: always-available slash commands. paper_search/book_search are opt-in Tools toggles (default OFF). After tool results, prefer the receipt answerMarkdown (one block per hit: bold `**N.**` index, title, meta, then each Download/Actions command as an unordered-list bullet) — same shape as slash output; never bury commands in em-dash prose. Papers: arXiv / Semantic Scholar / OpenAlex (+ clickable /papers details|citations|references <id> and /papers download when an open-access PDF is available — ARXIV:/pdf URL; never invent download ids for paywalled papers). Books: multi-source search; only emit /books download when the tool/slash receipt includes a real id (libgen md5 / IA id / gutenberg:id / direct URL) — never invent ids; if not downloadable, give a Manual download markdown link to the page.',
    ...
    '- file_read: 【历史文件引用】 markers carry fileId — call file_read with that file_id for a short page-unit slice (~8 by default). Extracts use --- page N --- units (PDF pages; PPTX/DOCX/XLSX/ZIP: page 1 = outline/catalog, later pages = slides/sections/sheets/members). Omit start_page to skip TOC/catalog when possible; pass start_page=1 or focus=目录/contents for the outline; ZIP focus may be a member path. Never invent file contents.',
    '- office_write / office_rollback: edit an existing .docx/.pptx/.xlsx in place (same file_id) when the user asked; auto-applies with snapshot_id + diff; Undo via office_rollback. Prefer structured ops over full_replace (requires confirm_full_replace:true). Never claim success without ok:true. create_file / create_spreadsheet still create NEW ids.',
