...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- office_write / office_rollback: edit an existing .docx/.pptx/.xlsx in place (same file_id) when the user asked; auto-applies with snapshot_id + diff; Undo via office_rollback. Prefer structured ops over full_replace. Never claim success without ok:true. create_file / create_spreadsheet still create NEW ids.',
