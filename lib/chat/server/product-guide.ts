...
export function productUsageGuidePrompt(): string {
  return [
    ...
    'Commands (composer “/”, sidebar Commands): /image <prompt> (client image gen — not a chat tool); /research [mode] [sources] <query> (Deep Research; sources=web|literature|mixed); /papers [arxiv|semantic|openalex] <query>; /papers details|citations|references <id>; /papers author <name>; /books [libgen|archive|gutenberg|fpb|aibooks|trading|github] <query>; /books download <archiveId|libgen:md5|url> (IA / LibGen / direct → Files); /skill [brief] (Skill Creator → save_skill); Request review / 请求审查; Continue reply / 继续回复.',
