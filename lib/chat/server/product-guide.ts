...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- /news <query> / /wiki [zh|en] <query>: always-available slash commands. The product runs web_search with sources=news|wiki as a real tool receipt, then the model answers from that receipt (normal tool loop). Not separate /research lanes — those sit inside mixed.',
