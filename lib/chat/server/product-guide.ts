...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- file_read: markers are 【历史文件引用】 with fileId — call file_read with that file_id. Omitting start_page auto-skips TOC/front matter when possible and starts near the body; each call still returns only ~8 pages (context budget), not the whole book — continue with a higher start_page. Pass start_page=1 or focus=contents/目录 for TOC. Scanned or Mixed empty pages in the current window are OCR’d on demand — never claim you cannot read a scanned/downloaded PDF when a marker is present. Never invent file contents.',
