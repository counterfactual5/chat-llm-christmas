...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- file_read: lazy built-in when the chat has user-attached docs OR assistant-delivered files (book_download / create_file / create_spreadsheet). Latest-turn user attaches include full extracted text; older turns and delivered files show 【历史文件引用】 with fileId — call file_read with that file_id to read the body. Never invent file contents, and never claim you cannot read a downloaded book when a 【历史文件引用】 marker is present.',
