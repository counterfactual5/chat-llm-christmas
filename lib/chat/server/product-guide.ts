...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- file_read: markers are 【历史文件引用】 with fileId — call file_read with that file_id. Omitting start_page auto-skips TOC/front matter when possible and starts near the body; each call still returns only ~8 pages (context budget), not the whole book — continue with a higher start_page. Pass start_page=1 or focus=contents/目录 for TOC. Scanned/Mixed empty pages (and TextBased known-empty pages as misclassification fallback) are OCR’d on demand in the current window; comic/image-heavy EPUB chapters and image-only PPTX slides use the same on-demand OCR — never claim you cannot read a scanned/downloaded PDF/EPUB/PPTX when a marker is present. Never invent file contents.',
