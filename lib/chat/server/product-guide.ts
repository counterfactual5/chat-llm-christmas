...
export function productUsageGuidePrompt(): string {
  return [
    ...
    'Tools (only if present in THIS-turn API tool list / capability flags): create_file (Output panel download); web_search/web_read built-in always-on (public web/news/docs — cite source+time; not a dedicated finance/market data feed); paper_search/book_search for academic/books (slash /papers|/books also work); generate_image (or /image command); image_understand on text-only models for prior-turn images marked 【历史图片引用（未转写）】 (vision chat models see images natively — no image_understand); save_skill only while Skill Creator is ON.',
...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- /image <prompt>: slash command OR generate_image chat tool (same image model). Prefer the tool when answering naturally; /image forces generation.',
    ...
    '- /papers / /books: slash commands AND paper_search/book_search chat tools. Papers: arXiv / Semantic Scholar / OpenAlex (+ /papers details/citations/references/author). Books: multi-source search; /books download <id> into Files.',
