...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- /news <query> / /wiki [zh|en] <query>: always-available slash commands. They search news/Wikipedia engines (same as web_search sources=news|wiki), then a tools-off model pass writes a readable briefing from the hits. Not separate /research lanes — those sit inside mixed.',
    '- /papers / /books: always-available slash commands (no Tools toggle needed). paper_search/book_search are opt-in Tools toggles (default OFF) so the model can search mid-reply without a slash — when OFF, still tell the user to type /papers or /books. Papers: arXiv / Semantic Scholar / OpenAlex (+ /papers details/citations/references/author). Books: multi-source search; /books download <id> into Files.',
