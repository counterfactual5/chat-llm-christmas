...
export function productUsageGuidePrompt(): string {
  return [
    ...
    'Commands (composer “/”, sidebar Commands): /image <prompt> (client image gen — not a chat tool); /research [quick|standard|rigorous] [web|literature|news|wiki|mixed] <query> (Deep Research; literature/mixed is thinner than dedicated /papers|/books; news/wiki reuse dedicated search paths); /news <query>; /wiki [zh|en] <query>; /papers [arxiv|semantic|openalex] <query>; /papers details|citations|references <id>; /papers author <name>; /books [libgen|archive|openlibrary|gutenberg|fpb|aibooks|trading|github] <query>; /books download <archiveId|libgen:md5|url> (IA / LibGen / direct → Files); /skill [brief] (Skill Creator → save_skill; command is /skill singular, not /skills); /review (Request review / 请求审查); Continue reply / 继续回复.',
    ...
    'Tools (only if present in THIS-turn API tool list / capability flags): create_file + web_search/web_read are built-in always-on (public web — cite source+time; web_search sources=news|wiki for headlines/encyclopedia; not a dedicated finance/market data feed). file_read is lazy (off until the chat has attached documents, then auto-ON) — re-read full text from a 【历史文件引用】 fileId when the short preview is not enough. paper_search / book_search / generate_image are OPT-IN Tools toggles (default OFF to save tokens) — when OFF, tell users to use slash /papers|/books|/image or enable the Tools switch; when ON and listed, call them naturally. image_understand is lazy for logged-in text-only models (off until the chat has images, then auto-ON); vision models see images natively. If image_understand is absent because there are no images yet, do NOT say you cannot understand images — invite the user to send one (never name internal tool/MCP/model ids). save_skill only while Skill Creator is ON.',
...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- /research [quick|standard|rigorous] [web|literature|news|wiki|mixed] <query>: Deep Research job (not an ordinary chat tool). Modes control depth; sources pick web vs literature vs news vs wiki vs mixed (mixed = web+literature only). Prefer /news or /wiki for dedicated retrieval; prefer /papers or /books for literature search/download.',
    '- /news <query> / /wiki [zh|en] <query>: always-available slash commands for news headlines and Wikipedia lookup (same engines as web_search sources=news|wiki).',
    '- /papers / /books: always-available slash commands (no Tools toggle needed). paper_search/book_search are opt-in Tools toggles (default OFF). Papers: arXiv / Semantic Scholar / OpenAlex (+ /papers details/citations/references/author). Books: multi-source search; /books download <id> into Files.',
