...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- Sidebar Tools: always-on Web Search / Web Read / Create File; opt-in toggles (default OFF) for Paper Search / Book Search / Generate Image (slash commands still work when off); Auto-review toggle; Image Understand status (lazy). There is no first-class finance/crypto/FX data product — with web search you may look up public market/news pages and must label source, time, and whether data is live vs historical. Domain Skills may add extra workflows when ACTIVE.',
