...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- Sidebar Tools: always-on Web Search / Web Read / Create File (+ create_spreadsheet for real .xlsx); opt-in toggles (default OFF) for Paper Search / Book Search / Generate Image (slash commands still work when off); Auto-review toggle; Image Understand status (lazy); file_read status (lazy when documents are attached). There is no first-class finance/crypto/FX data product — with web search you may look up public market/news pages and must label source, time, and whether data is live vs historical. Domain Skills may add extra workflows when ACTIVE.',
    ...
    '- file_read: lazy built-in when the chat has user-attached docs OR assistant-delivered files (book_download / create_file / create_spreadsheet). Markers are 【历史文件引用】 with fileId — call file_read with that file_id. Default return is a short page slice (~8 pages from body start when TOC can be skipped via PDF outline or text heuristic), not the whole book; pass start_page=1 or focus=contents/目录 to read the TOC; use start_page / max_pages / focus to continue. Never invent file contents, and never claim you cannot read a downloaded book when a marker is present.',
    '- docx_extract / xlsx_extract: also lazy when attachments exist. Prefer these to open a specialized Output/sidebar view (docx.extract|outline|comments, xlsx.table). file_read = plain text for reasoning; extract tools = structured UI for the user. If ok:true but empty:true, say the view opened with no content and suggest another mode/sheet.',
