...
export function productUsageGuideDetailPrompt(): string {
  return [
    ...
    '- /research [quick|standard|rigorous] [web|literature|mixed] <query>: Deep Research job (not an ordinary chat tool). Modes control depth; sources pick web vs literature-only vs mixed (mixed = web + papers/books + news + wiki). Prefer /news or /wiki for one-shot retrieval; prefer /papers or /books for dedicated literature search/download.',
    '- /news <query> / /wiki [zh|en] <query>: always-available slash commands for news headlines and Wikipedia lookup (same engines as web_search sources=news|wiki). Not separate /research lanes — those sit inside mixed.',
