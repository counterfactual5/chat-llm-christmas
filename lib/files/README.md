...
| `ingest/` | Client-side extract + prepare before upload (PDF/DOCX/PPTX/ZIP/Excel/EPUB/text/images). Primary converter goes through `@firecrawl/anydoc-wasm` for DOCX/PPTX/PDF (lazy-loaded, ~6MB), falling back to per-format JS extractors (`ingest/extractors/{pdf,epub,docx,pptx,spreadsheet,zip}.ts`) on `unsupported`/`malformed`/`encrypted`/`resourceLimit`/`missingPart`. Paging contract shared with future server-side usage: `ingest/anydoc-paging.ts` + [`docs/anydoc-paging.md`](../../docs/anydoc-paging.md). Drop whitelist / ZIP member kinds: `ingest/support.ts`. |
