...
| `ephemeral-preview.ts` | Shared ephemeral literature Preview entries (`paper-preview:` / `book-preview:`) + same-origin content URL builders for `/api/literature/papers|books/content`. Does **not** write Files. |
| `literature-preview-ladder.ts` | Shared paper/book Preview ladder (`attemptLiteratureEphemeralPreview`, probe/mime helpers, `persistLiteraturePreview` / `persistEphemeralLiteratureEntry`). UrlPreviewPanel + chat-container Download must use this — do not fork paper vs book paths. |
| `url-preview.ts` | Side-panel online URL helpers (`isPreviewableHttpUrl`, normalize, `resolvePreviewHttpUrl`, same-document nav equality, external-click detection, `isLikelyAuthGatedPreviewUrl`, `isLikelyPaperPreviewUrl` / `isLikelyBookPreviewUrl` / `isJunkBookExtractHost` / `isLikelyDirectLiteratureFileUrl`). Paper/book ladder: resolve+probe → ephemeral reader when streamable; Save to Files & open when downloadable only; otherwise HTML extract via AnswerMarkdown (libgen / library.lol junk landings stay CTA-only, never extract). Direct `.pdf` / `/pdf` / `.epub` / `.djvu` gates into literature preview. Extract Text links navigate in-panel via `onPreviewLink`. Explicit `/papers|/books download` (or Preview Save) persists with `source_key` dedupe. Image Describe / OCR in extract is **manual only** (`UrlPreviewImage`). |
