...
| `url-preview.ts` | Side-panel online URL helpers (`isPreviewableHttpUrl`, normalize, `resolvePreviewHttpUrl` for extract/markdown hrefs + base, same-document nav equality, external-click detection, `isLikelyAuthGatedPreviewUrl` for hosts that need top-level browser login, `isLikelyPaperPreviewUrl` / `isLikelyBookPreviewUrl` for OA resolve-before-HTML). Book gate includes archive.org, Gutenberg, and Library Genesis (`libgen.*` / `library.lol` / `ads.php?md5=`). Extract Text links navigate in-panel via `onPreviewLink`; cross-origin iframe clicks stay uninterceptable. Paper/book Preview resolves then probes content before opening ephemeral readers (does **not** write Files). Explicit `/papers|/books download` (or Preview Save / ephemeral Download) persists with `source_key` dedupe. Thin/paywalled / libgen landing HTML shows a CTA instead of junk extract. Webpage image OCR is out of scope for URL Preview. |
