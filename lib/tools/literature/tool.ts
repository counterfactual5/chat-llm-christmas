...
export const LITERATURE_TOOL_ANSWER_HINT = {
  books:
    ...
    'Each hit is one block: bold index (`**N.**`) + title link, then meta (authors · year · source · size), ' +
    'then each Download / Manual download command as an unordered-list bullet in backticks. ' +
  ...
  papers:
    ...
    'Each hit is one block: bold index (`**N.**`) + title link, then meta (authors · year · venue · ID), ' +
    'then each `/papers details|citations|references|download` command as an unordered-list bullet in backticks ' +
    '(markdown hard breaks between title/meta only; no blank lines between bullets). ' +
    'Blank line only before the next `N.`. ' +
    'When downloadCommand is present, emit that command as its own bullet in backticks — that stores the PDF in Files; ' +
...
export const PAPER_SYSTEM = [
  ...
  'After results, match answerMarkdown / the /papers slash shape: one block per hit — bold index (`**N.**`) + title link, meta line, then each `/papers details|citations|references|download` command as an unordered-list bullet in backticks (hard breaks between title/meta only; no blank lines between bullets) — never invent ids, never bury commands in em-dash prose.',
  ...
  'downloadCommand saves the PDF into the user Files store (in-app). Emit it as its own bullet in backticks — do not turn an external URL into a “download” link.',
...
export const BOOK_SYSTEM = [
  ...
  'After results, match answerMarkdown / the /books slash shape: one block per hit — bold index (`**N.**`) + title link, meta line, then each `/books download …` / Manual download as an unordered-list bullet in backticks — never invent identifiers, never bury commands in em-dash prose.',
  'Blank line before the next numbered hit.',
  'When downloadCommand is absent, put Manual download: [title](url) as a bullet under the meta. Prefer copying answerMarkdown from the receipt when present.',
