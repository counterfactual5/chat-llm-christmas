...
export function DocxCommentsView({ view }: { view: ToolViewPayload }) {
  ...
  if (!comments.length) {
    return <p className="px-4 py-6 text-xs text-stone-400">No comments.</p>;
  }
  return (
      ...
      {comments.map((c, i) => (
        ...
        >
          <div className="mb-1 flex flex-wrap items-baseline gap-x-2 text-[11px] text-stone-400">
            {c.author ? <span className="font-medium text-stone-600 dark:text-stone-300">{c.author}</span> : null}
